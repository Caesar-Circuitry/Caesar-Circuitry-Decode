...
public class FlywheelKinematics {
  ...
  public double calculateArtifactVelocity() {
    targetVelocity =
        Math.sqrt(
            (gravity * Math.pow(xlaunch, 2))
                / ((2 * Math.pow(Math.cos(LaunchAngle), 2))
                    * (xlaunch * Math.tan(LaunchAngle) - (hgoal - hbot))));
    ...
    targetVelocity =
        Math.sqrt(
            (gravity * Math.pow(xlaunch, 2))
                / ((2 * Math.pow(Math.cos(LaunchAngle), 2))
                    * (xlaunch * Math.tan(LaunchAngle) - (hgoal - hbot))));
    double launchVelocity = targetVelocity; // TODO Insert Regression Formula HERE
