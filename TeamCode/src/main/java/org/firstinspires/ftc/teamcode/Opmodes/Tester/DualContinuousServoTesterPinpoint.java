...
public class DualContinuousServoTesterPinpoint extends LinearOpMode {
  ...
  public static double kP = 0.01, kI = 0.0, kD = 0.001;
  public static double kF_left = 0.0; // Feedforward when turning left (positive error)
  public static double kF_right = 0.0; // Feedforward when turning right (negative error)
  ...
  public void runOpMode() throws InterruptedException {
    ...
    while (opModeIsActive()) {
      ...
      // If robot is at 60 degrees and we want turret to point at 0 degrees field angle,
      // then turret needs to be at -60 degrees relative to robot
      ...
      // This considers both the direct path and the wrap-around path
      // The turret has a physical limit of 270 degrees centered at 0
      ...
      // Encoder is mounted on servo, so it reads servo position
      // gearRatio = servo rotation / turret rotation (e.g., 2:1 means servo rotates 2x as much)
      // Therefore: servo angle = turret angle * gear ratio
      double desiredServoAngle = desiredTurretAngle * gearRatio;

      // Read current servo angle from absolute encoder (in degrees)
      // Encoder is based on servo position, not turret position
      // getUnwrappedServoAngle returns the servo angle directly (no conversion needed)
      ...
      // We need to handle angle wrapping manually to ensure shortest path
      ...
      // This ensures PID takes the shortest path
      double adjustedTarget = currentServoAngle + wrappedServoError;

      // Use PID controller with current position and adjusted target
      double servoPower = angleController.calculate(currentServoAngle, adjustedTarget);

      // Add direction-specific feedforward term
      // Left (positive error) uses kF_left, right (negative error) uses kF_right
      if (wrappedServoError > 0) {
        servoPower += kF_left;
      } else if (wrappedServoError < 0) {
        servoPower -= kF_right; // Subtract because power should be negative for right
      }
